...
export function FinalizeDialog({ open, onOpenChange }: FinalizeDialogProps) {
  ...
  // Actual unreviewed risks sorted by severity (for the expandable list)
  const unreviewedRisks = useMemo(() => {
    const reviewedParaIds = new Set(Object.keys(revisions));
    const severityOrder: Record<string, number> = { critical: 0, high: 1, medium: 2, low: 3, info: 4 };
    return risks
      .filter((r) => !reviewedParaIds.has(r.para_id))
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl">
        ...
        {/* Unaddressed risks expandable list */}
        {unreviewedRisks.length > 0 && (
          <div className="rounded-md border border-amber-200 bg-amber-50/50 dark:border-amber-800 dark:bg-amber-950/20">
            <Accordion type="single" collapsible>
              <AccordionItem value="unreviewed" className="border-0">
                <AccordionTrigger className="px-3 py-2 text-xs font-medium text-amber-800 hover:no-underline dark:text-amber-200">
                  <span className="flex items-center gap-2">
                    <AlertTriangle className="h-3.5 w-3.5" />
                    {unreviewedRisks.length} unaddressed risk{unreviewedRisks.length !== 1 ? "s" : ""} — review before exporting?
                  </span>
                </AccordionTrigger>
                <AccordionContent className="px-3 pb-3">
                  <div className="max-h-48 space-y-1 overflow-y-auto">
                    {unreviewedRisks.map((risk) => (
                      <div key={risk.risk_id} className="flex items-center gap-2 text-xs">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            useAppStore.getState().selectParagraph(risk.para_id);
                            onOpenChange(false);
                          }}
                        >
                          Go to
...
function SeverityDot({ severity }: { severity: string }) {
  ...
  return (
    <span
      className={`h-2 w-2 shrink-0 rounded-full ${colorMap[severity] ?? "bg-gray-400"}`}
      aria-label={severity}
    />
  );
}
