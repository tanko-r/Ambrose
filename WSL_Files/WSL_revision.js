...
function showBottomSheet(paraId) {
    ...
    // Show related suggestions (AI suggestions for other clauses that may need attention)
    showRelatedSuggestionsPanel(paraId, revision);
...
// Show related suggestions panel (AI suggestions for other clauses that may need revision)
function showRelatedSuggestionsPanel(paraId, revision) {
    // Get or create the related suggestions panel
    let panel = document.getElementById('related-suggestions-panel');
    if (!panel) {
        panel = document.createElement('div');
        panel.id = 'related-suggestions-panel';
        panel.className = 'related-suggestions';
        // Insert after related-revisions-panel or after revision-actions
        const relRevisionsPanel = document.getElementById('related-revisions-panel');
        const actionsEl = document.getElementById('revision-actions');
        if (relRevisionsPanel) {
            relRevisionsPanel.parentNode.insertBefore(panel, relRevisionsPanel.nextSibling);
        } else if (actionsEl) {
            actionsEl.parentNode.insertBefore(panel, actionsEl);
        }
    }

    const suggestions = revision.relatedSuggestions || [];

    if (suggestions.length === 0 || revision.accepted) {
        panel.style.display = 'none';
        return;
    }

    panel.innerHTML = renderRelatedSuggestions(suggestions, paraId);
    panel.style.display = 'block';
}

// Render related suggestions HTML
function renderRelatedSuggestions(suggestions, paraId) {
    if (!suggestions || suggestions.length === 0) {
        return '';
    }

    let html = '<div class="related-suggestions-header">Related Suggestions</div>';

    suggestions.forEach((sug, idx) => {
        const priorityClass = sug.priority === 'recommended' ? 'recommended' : 'optional';
        const sectionDisplay = sug.section || sug.para_id || 'Unknown';
        const suggestionText = sug.suggestion || sug.description || '';
        const sugParaId = sug.para_id || '';

        html += `
            <div class="related-suggestion ${priorityClass}" data-suggestion-idx="${idx}">
                <div class="related-suggestion-header">
                    <span class="related-suggestion-section">${escapeHtml(sectionDisplay)}</span>
                    <span class="related-suggestion-priority">${sug.priority === 'recommended' ? 'Recommended' : 'Optional'}</span>
                </div>
                <div class="related-suggestion-text">${escapeHtml(suggestionText)}</div>
                ${sugParaId ? `
                    <div class="related-suggestion-actions">
                        <button class="btn btn-sm btn-secondary" onclick="navigateToSuggestion('${sugParaId}')">
                            View Clause
                        </button>
                        <button class="btn btn-sm btn-primary" onclick="generateRelatedRevision('${sugParaId}', '${paraId}')">
                            Generate Revision
                        </button>
                    </div>
                ` : ''}
...
// Navigate to a suggested clause
function navigateToSuggestion(paraId) {
    hideBottomSheet();
    selectParagraph(paraId);
}

// Generate revision for a related clause based on a suggestion
async function generateRelatedRevision(relatedParaId, sourceParaId) {
    showToast('Generating revision for related clause...', 'info');

    try {
        // Get all risk IDs for the related paragraph
        const relatedRisks = AppState.analysis?.risk_by_paragraph?.[relatedParaId] || [];
        const riskIds = relatedRisks.map(r => r.risk_id);

        if (riskIds.length === 0) {
            // No identified risks, just navigate to it
            showToast('No risks identified for this clause. Navigate to review manually.', 'info');
            hideBottomSheet();
            selectParagraph(relatedParaId);
            return;
        }

        // Generate revision for the related paragraph
        await generateRevisionForRisks(relatedParaId, riskIds, [sourceParaId]);

        // Navigate to the related paragraph
        hideBottomSheet();
        selectParagraph(relatedParaId);
        showToast('Revision generated for related clause', 'success');

    } catch (error) {
        showToast(error.message || 'Failed to generate revision', 'error');
    }
}
...
async function acceptRevision(paraId) {
    try {
        ...
        // Show affected clauses notification if any concepts changed
        if (result.affected_para_ids && result.affected_para_ids.length > 0) {
            const para = AppState.document?.content?.find(p => p.id === paraId);
            const sectionRef = para?.section_ref || paraId;
            showAffectedClausesNotification(result.affected_para_ids, sectionRef);
        }
...
window.renderRelatedSuggestions = renderRelatedSuggestions;
window.showRelatedSuggestionsPanel = showRelatedSuggestionsPanel;
window.navigateToSuggestion = navigateToSuggestion;
window.generateRelatedRevision = generateRelatedRevision;
