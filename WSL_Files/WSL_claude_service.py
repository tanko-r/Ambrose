...
def analyze_document_with_llm(
    ...
    Raises:
        RuntimeError: If Anthropic SDK is not installed or API key is missing
    """
    # Fail early if prerequisites are not met
    if not HAS_ANTHROPIC:
        raise RuntimeError("Anthropic SDK not installed. Run: pip install anthropic")
    ...
    for i in range(0, len(paragraphs), batch_size):
        ...
        try:
            ...
            # Fail the entire analysis on error - no silent continuation
            if session_id:
                update_progress(session_id, {
                    'status': 'error',
                    'last_error': str(e),
                    'current_action': f'Error in batch {batch_num}'
                })
            raise RuntimeError(f"Analysis failed at batch {batch_num}: {e}")
...
Analyze this clause and return a JSON array of risks."""
    ...
    try:
        ...
        response_text = response.content[0].text if response.content else "[]"
        parsed = parse_risk_response(response_text)
