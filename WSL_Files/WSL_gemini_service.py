...
GET_CLAUSE_TOOL = {
    "name": "get_clause",
    "description": "Retrieve full text of a clause by section reference. Use when you need to see the exact language of a provision referenced in the concept map or risk map.",
    "parameters": {
        "type": "object",
        "properties": {
            "section_ref": {
                "type": "string",
                "description": "Section reference like '8.3' or 'Section 8.3'"
            },
            "reason": {
                "type": "string",
...
def handle_tool_call(
    ...
    section_ref = args.get('section_ref', '').strip()

    # Normalize section ref
    normalized = section_ref.lower().replace('section ', '').strip()

    # Search for matching paragraph
    for para in parsed_doc.get('content', []):
        if para.get('type') != 'paragraph':
            continue

        para_ref = (para.get('section_ref') or '').lower().strip()
        if para_ref == normalized or para_ref.endswith(normalized):
            return f"## Section {para.get('section_ref', section_ref)}\n\n{para.get('text', '')}"

    return f"Clause {section_ref} not found in document."
    ...
    try:
        ...
        if text.startswith('{'):
            ...
            return {
                ...
                'related_suggestions': data.get('related_suggestions', [])
            }
...
def generate_revision(
    ...
    Args:
        ...
        concept_map: Optional document-wide concept map with provisions
        risk_map: Optional risk map with dependency relationships
        parsed_doc: Optional parsed document for tool calls (get_clause)

    Returns:
        Dict with revised_text, rationale, thinking, and diff_html.
    """
    ...
    # Configure tools if parsed_doc is available for clause lookups
    tools = None
    if parsed_doc:
        tools = [GET_CLAUSE_TOOL]
        # TODO: Implement full Gemini function calling loop
        # The complete implementation would:
        # 1. Add tools to the generation config
        # 2. Check response for FunctionCall parts
        # 3. Execute handle_tool_call() with the requested args
        # 4. Send tool results back to Gemini
        # 5. Repeat until Gemini returns final text
        # For now, tools are configured but multi-turn handling is not implemented
