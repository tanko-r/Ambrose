...
<task type="auto">
  <name>Task 2: Sidebar overlay/collapse + Related tab opens precedent + snippet badge</name>
  <files>frontend/src/components/review/sidebar.tsx, frontend/src/components/review/related-clauses-tab.tsx</files>
  <action>
...
**Snippet badge on Generate button**:
...
1. Import `openPrecedentPanel` from store and `useAppStore`.
2. Each clause card becomes clickable:
   - On click: store the target clause paraId (so PrecedentPanel knows where to scroll), then call `openPrecedentPanel()`.
   - To pass the scroll target: add a new store field `precedentScrollTarget: string | null` and `setPrecedentScrollTarget(paraId)` action (add to store in this task). Set it before opening the panel. PrecedentPanel reads it on mount and scrolls to it, then clears it.
...
Verify related-clauses-tab.tsx calls `openPrecedentPanel` and does NOT show similarity percentages.
  ...
  <done>
Sidebar auto-collapses when precedent opens, shows left-edge restore tab, expands as z-40 overlay without closing precedent. Generate button shows snippet badge count with removal dropdown. Related clauses tab cards are clickable to open precedent scrolled to that clause. "Open Precedent" link shown when no matches. Similarity percentages removed. TypeScript compiles.
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 3: End-to-end verification of precedent split view</name>
  <files>none</files>
  <action>
