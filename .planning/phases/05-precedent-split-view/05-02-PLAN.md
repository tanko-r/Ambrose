...
7. Render three action buttons in a compact horizontal bar:
   ...
   - **Flag for Reference**: Call `flagItem` from API (or store action). Create an attorney flag with note referencing the precedent clause. Toast: "Flagged for reference". This creates a flag on the currently selected TARGET paragraph, with the note including the precedent section ref.
...
Grep for `PrecedentContent` and `PrecedentSelectionTooltip` to confirm exports.
  ...
  <done>
PrecedentContent renders backend HTML with dangerouslySetInnerHTML, attaches imperative click handlers via requestAnimationFrame, applies related-clause highlight classes, exposes scrollToClause via forwardRef. PrecedentSelectionTooltip shows floating actions (Copy, Use in Revision, Flag for Reference) on text selection using @floating-ui/react-dom virtual element positioning. TypeScript compiles.
  </done>
</task>
