...
must_haves:
  ...
  key_links:
    ...
    - from: "app/static/js/document.js"
      to: "selectParagraph"
      via: "click handler on PDF overlay"
...
<task type="auto">
  <name>Task 2: Implement PDF rendering in document.js</name>
...
4. **Modify loadDocument() to try PDF first:**
```javascript
...
6. **Update selectParagraph() to handle PDF mode:**
Add this at the start of selectParagraph():
```javascript
// Update PDF overlay selection if in PDF mode
if (pdfRenderMode) {
    document.querySelectorAll('.pdf-para-region.selected').forEach(el =>
...
3. Click on different areas of the PDF - sidebar should update
4. Verify paragraph selection visual feedback works
5. Check console for any errors
  </verify>
  <done>
Main document panel renders PDF using PDF.js. Paragraph overlay allows click-to-select. Selection triggers sidebar update. Fallback to text rendering if PDF fails.
  </done>
</task>
...
- [ ] Click on PDF selects paragraph and updates sidebar
...
4. Sidebar updates with risk analysis for selected paragraph
