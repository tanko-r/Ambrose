...
must_haves:
  ...
  key_links:
    ...
    - from: "app/static/js/precedent.js"
      to: "copy event listener"
      via: "text layer enables selection, copy handler provides feedback"
      pattern: "addEventListener.*copy|setupPrecedentCopyHandler"
---
...
<task type="auto">
  <name>Task 3: Add text selection layer with copy handler for PDF (PREC-04)</name>
  <files>app/static/js/precedent.js, app/static/css/main.css</files>
  <action>
PDF.js renders to canvas which doesn't support native text selection. Add a text layer for selection support and wire up the copy handler:
...
3. **CRITICAL: Add copy event handler for PDF mode:**
The text layer enables selection, but we need to wire up copy handling with visual feedback.

Add this function to precedent.js:
```javascript
function setupPrecedentPdfCopyHandler() {
    // Remove any existing handler to avoid duplicates
    if (window._precedentCopyHandler) {
        document.removeEventListener('copy', window._precedentCopyHandler);
    }

    window._precedentCopyHandler = (e) => {
        const selection = window.getSelection();
        if (!selection || !selection.toString().trim()) return;

        // Check if selection is within precedent panel (either text layer or text content)
        const precedentContent = document.getElementById('precedent-content');
        const pdfContainer = document.getElementById('precedent-pdf-container');

        let isInPrecedent = false;
        if (precedentContent && precedentContent.contains(selection.anchorNode)) {
            isInPrecedent = true;
        }
        if (pdfContainer && pdfContainer.contains(selection.anchorNode)) {
            isInPrecedent = true;
        }
        ...
        if (isInPrecedent) {
            ...
            if (typeof showToast === 'function') {
                ...
                console.log('Precedent text copied:', selection.toString().substring(0, 50) + '...');
            }
        }
    };

    document.addEventListener('copy', window._precedentCopyHandler);
}
```

4. **Call the copy handler setup after PDF renders:**
At the end of renderPrecedentAsPdf(), after `precedentPdfRenderMode = true;`, add:
```javascript
// Set up copy handler for PDF text layer (PREC-04)
setupPrecedentPdfCopyHandler();
```

5. **Also set up copy handler for text fallback:**
In renderPrecedentAsText(), ensure setupPrecedentCopyHandler() is called (it should already be there).

6. **Clean up copy handler when closing panel:**
In closePrecedentPanel(), add:
```javascript
// Clean up copy handler
if (window._precedentCopyHandler) {
    document.removeEventListener('copy', window._precedentCopyHandler);
    window._precedentCopyHandler = null;
}
```
  </action>
  <verify>
1. Open precedent panel with PDF rendering
2. Try to select text in the PDF (click and drag over text)
3. Press Ctrl+C to copy
4. Paste in another application - verify text is copied
5. Toast notification should appear (if showToast exists)
6. Verify copy also works in text fallback mode
  </verify>
  <done>
Text selection works in PDF mode via text layer overlay. Copy event handler is wired up and provides visual feedback. Users can select and copy text from precedent document (PREC-04 requirement met).
  </done>
</task>
...
- [ ] Copy/paste works with feedback (PREC-04) - copy handler wired up
...
5. Copy action shows toast feedback (if available)
6. Navigation panel clicks scroll PDF to correct section
