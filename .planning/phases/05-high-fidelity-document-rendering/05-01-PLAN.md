...
IMPORTANT: Do NOT use concurrent LibreOffice calls - the library cannot handle them. This is acceptable for single-user local app usage.
  ...
  <verify>
**Setup**: First, ensure LibreOffice is installed at a standard location.

**Test LibreOffice detection**:
```bash
python -c "from app.services.pdf_service import libreoffice_exec; print(libreoffice_exec())"
```
Should print path to soffice.exe (e.g., `C:\Program Files\LibreOffice\program\soffice.exe`).
...
# Use any existing DOCX in the project (e.g., from test_data or uploads)
test_docx = Path("app/data/uploads").glob("**/*.docx").__next__()
...
Run with: `python test_pdf.py`
  ...
  <done>
pdf_service.py exists with all three functions. libreoffice_exec() returns valid path on Windows. convert_docx_to_pdf() successfully converts a DOCX to PDF.
  </done>
</task>
...
Same logic but uses session['precedent_path'] instead.
  ...
  <verify>
**Setup**: First create a test session by uploading a document through the app.

1. Start the Flask server: `python -m app.server`
2. Open browser to `http://localhost:5000`
3. Upload a test DOCX file through the intake flow
4. Note the session_id from the URL or response

**Test the endpoint**:
```bash
# Replace {SESSION_ID} with actual session ID from step 3
curl -I http://localhost:5000/api/document/{SESSION_ID}/pdf
```
Expected: `HTTP/1.1 200 OK` with `Content-Type: application/pdf`

**Test PDF opens correctly**:
Open `http://localhost:5000/api/document/{SESSION_ID}/pdf` in browser - PDF should display.
  </verify>
  <done>
