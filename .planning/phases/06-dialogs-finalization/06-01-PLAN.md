...
<task type="auto">
  <name>Task 2: Flags tab rewrite + document margin icons</name>
  <files>
    frontend/src/components/review/flags-tab.tsx
    frontend/src/app/globals.css
  </files>
...
- Document margin shows color-coded flag icons for flagged paragraphs via CSS
  </done>
</task>

<task type="auto">
  <name>Task 3: Text selection flagging + document margin data attributes</name>
  <files>
    frontend/src/components/review/document-viewer.tsx
  </files>
  <action>
...
1. **Flag margin icon data attributes:** In `updateParagraphStates`, when toggling the `.flagged` class, also set a `data-flag-category` attribute on the element with the flag's category value. If the paragraph has multiple flags, use the first flag's category. If unflagged, remove the `data-flag-category` attribute. This enables the CSS from Task 2 to color-code the margin icon.
...
- Clicking floating Flag button opens FlagDialog with correct paraId
  </verify>
  <done>
- Flagged paragraphs have `data-flag-category` attribute enabling CSS margin icons
- Text selection in document viewer shows floating Flag button that opens FlagDialog
- Selection context (paraId + text excerpt) correctly identified from DOM
  </done>
</task>
