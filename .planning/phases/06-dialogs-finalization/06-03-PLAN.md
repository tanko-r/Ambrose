...
must_haves:
  ...
  key_links:
    ...
    - from: "frontend/src/components/review/sidebar.tsx"
      ...
      via: "FlagDialog opened from sidebar Flag icon button"
      pattern: "FlagDialog"
---

<objective>
Build the transmittal email dialog, enhance the new project workflow (auto-save, skip preference, status badges, delete), add finalized project banner, wire transmittal button in bottom bar, and add sidebar quick-flag button. Also enhance backend endpoints for transmittal and session deletion.

Purpose: These dialogs complete the full review-to-delivery workflow. The transmittal is the client communication artifact. The new project flow ensures smooth session transitions. The finalized banner prevents accidental edits to completed reviews. The sidebar flag button enables quick flagging during risk review.
...
<task type="auto">
  <name>Task 1: Transmittal dialog + backend transmittal enhancement + bottom bar wiring</name>
  <files>
    frontend/src/components/dialogs/transmittal-dialog.tsx
    frontend/src/lib/api.ts
    frontend/src/components/review/bottom-bar.tsx
...
NOTE: The session DELETE file cleanup is handled in Task 2, not here.
...
<task type="auto">
  <name>Task 2: New project enhancement + delete dialog + recent projects + finalized banner + sidebar flag button + session DELETE cleanup</name>
  <files>
    frontend/src/components/dialogs/new-project-dialog.tsx
    frontend/src/components/dialogs/delete-project-dialog.tsx
    frontend/src/components/dashboard/recent-projects.tsx
...
- Import `FlagDialog` from `@/components/dialogs/flag-dialog` (created by Plan 06-01).
...
- Clicking sidebar Flag button opens FlagDialog
  </verify>
  <done>
...
- Backend session DELETE removes JSON file from disk
- Sidebar has quick-flag button opening FlagDialog for selected paragraph
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <what-built>
Complete dialog suite: Flag system (4 categories, sidebar + text selection + FlagsTab entry points, margin icons), Finalize dialog (stats, revision list, author name, Word export), Transmittal dialog (editable email, copy, mailto), New Project enhancement (auto-save, skip preference, status badges, delete), Finalized project banner, Sidebar quick-flag button.
  </what-built>
  <action>Human verification checkpoint. No code changes -- user tests all Phase 6 features end-to-end per the verification steps below.</action>
  <how-to-verify>
...
30. Click "Edit" on the banner. Verify banner disappears and session is fully editable.
  ...
  <verify>Human confirms all 30 verification steps pass or reports specific issues for each area</verify>
  <done>All Phase 6 features verified: flags (3 entry points + margin icons), finalize (stats + export + download), transmittal (edit + copy + mailto), new project (auto-save + skip + badges + delete), finalized banner (shows + Edit clears)</done>
...
6. Sidebar flag button: opens FlagDialog from risk card footer
7. `npx tsc --noEmit` passes
8. No console errors during all workflows
9. Backend: transmittal includes categories and revision summary, session DELETE removes file
</verification>
...
- Sidebar quick-flag button works for selected paragraphs
</success_criteria>
